using BlockBase.Dapps.NeverForgetBot.Business.Interfaces;
using BlockBase.Dapps.NeverForgetBot.Dal.GenericDataAccess.Interfaces;
using BlockBase.Dapps.NeverForgetBot.Data.Entities;
using System;

namespace BlockBase.Dapps.NeverForgetBot.Business.GenericBusiness
{
    public class TwitterCommentBo : BaseBo<TwitterComment>, ITwitterCommentBo
    {
        public TwitterCommentBo(IBaseAuditDao<TwitterComment, Guid> dao, IDbOperationExecutor opExecutor) : base(dao, opExecutor)
        {
        }
    }
}
